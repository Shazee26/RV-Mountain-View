...
const Rates: React.FC = () => {
  return (
    <section id="rates" className="py-24 bg-stone-900 text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold mb-4">Transparent Pricing</h2>
          <p className="text-stone-400 max-w-2xl mx-auto text-lg">
            No hidden fees. Just great sites and friendly service.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {RATES.map((rate, idx) => (
            <div
              key={idx}
              className={`relative bg-stone-800 p-8 rounded-3xl border ${
              ...
              <ul className="space-y-4 mb-8">
                {rate.details.map((detail, i) => (
                  <li key={i} className="flex items-center gap-3 text-stone-300">
                    <Icon name="Check" className="text-emerald-500" size={18} />
                    {detail}
                  </li>
                ))}
              </ul>
        ...
        <div className="mt-16 text-center text-stone-500 italic">
          *Monthly rates do not include electricity which is metered and billed separately.
        </div>
      </div>
    </section>
  );
